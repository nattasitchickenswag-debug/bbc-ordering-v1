...
export default function BBCSystemFinal() {
  ...
  const menuData = useMemo(() => {
    ...
    const chickenItems = [
      ...
      { id: "F0027", name: "ขาไก่พะโล้", unit: "ถุง", icon: <UtensilsCrossed size={16}/>, type: "all", sortOrder: 5 },
    ...
    const chickenSaucesAndOils = [
      { id: "F0022", name: "น้ำจิ้มไก่ทอด (4300 มล./แกลลอน)", unit: "แกลลอน", icon: <ChefHat size={16}/>, type: "chicken", sortOrder: 14 },
    ...
    return {
      ...
      "🍗 วัตถุดิบหลัก": [
        ...
        { id: "F0027", name: "ขาไก่พะโล้", unit: "ถุง", icon: <UtensilsCrossed size={16}/>, type: "all", sortOrder: 9 },
        { id: "F0034", name: "ไก่พริกไทยดำ", unit: "ถุง", icon: <UtensilsCrossed size={16}/>, type: "chicken", sortOrder: 10 },
      ],
      "🥬 ผักสด": [
      ...
      "🍳 เครื่องปรุง": [
        ...
        { id: "F0022", name: "น้ำจิ้มไก่ทอด (4300 มล./แกลลอน)", unit: "แกลลอน", icon: <ChefHat size={16}/>, type: "chicken", sortOrder: 12 },
      ],
      "📦 บรรจุภัณฑ์": [
      ...
      "🧼 อื่นๆ": [
        ...
        { id: "G0004", name: "ถุงมือยาง L", unit: "แพ็ค", icon: <Droplets size={16}/>, type: "all", sortOrder: 2 },
      ...
      "📜 ประวัติการสั่ง": [] // Tab ใหม่สำหรับประวัติ
    };
  }, [selectedBranch]);

  // ฟังก์ชันดึงประวัติการสั่ง
  const fetchHistory = async () => {
    if (!selectedBranch) return;
    setHistoryLoading(true);
    try {
      // ตรงนี้ต้องเปลี่ยน URL เป็น Webhook GET ของ n8n ที่คุณทำไว้
      const response = await fetch(`https://nattasitpsk.app.n8n.cloud/webhook/your-history-endpoint?branch=${selectedBranch.branchName}`);
  ...
  const handleOrder = async () => {
    ...
    const payload = {
      ...
      items: Object.keys(cart).filter(id => parseFloat(cart[id].qty) > 0).map(id => ({ 
        sku: id, 
        name: cart[id].name, 
        qty: parseFloat(cart[id].qty), 
        stock: parseFloat(cart[id].stock) || 0 
      })),
      remark: orderRemark,
  ...
  const filteredItems = useMemo(() => {
    ...
    if (activeTab === "📜 ประวัติการสั่ง") return []; // หน้าประวัติไม่ต้องใช้ filteredItems

    return (menuData[activeTab] as any[])
      .filter(item => {
  ...
  useEffect(() => {
    ...
    const payload = { staffCode, selectedBranch, activeTab, cart, orderRemark, step };
  ...
  if (step === 2) return (
    <div className="min-h-screen bg-slate-50 pb-40 font-sans text-slate-900">
          ...
          {Object.keys(menuData).map(tab => (
            <button 
              key={tab} onClick={() => { 
                ...
                setActiveTab(tab); 
                setSearchQuery("");
                if (tab === "📜 ประวัติการสั่ง") fetchHistory();
              }}
      ...
      <div className={`p-2 pb-20 grid gap-2 max-w-4xl mx-auto ${activeTab === "รายการสั่งประจำ" ? "grid-cols-2 md:grid-cols-3 lg:grid-cols-4" : (activeTab === "📜 ประวัติการสั่ง" ? "grid-cols-1" : "grid-cols-2")}`}>
        
        {/* แสดงหน้าประวัติการสั่ง */}
        {activeTab === "📜 ประวัติการสั่ง" ? (
          <div className="space-y-3">
            {historyLoading ? (
              <p className="text-center py-20 text-slate-400 font-bold animate-pulse text-xs uppercase tracking-widest">กำลังโหลดประวัติ...</p>
            ) : orderHistory.length === 0 ? (
              <div className="text-center py-20">
                <History size={48} className="mx-auto text-slate-200 mb-2" />
                <p className="text-slate-400 font-bold text-xs uppercase tracking-widest">ยังไม่มีประวัติการสั่ง</p>
              </div>
            ) : (
              orderHistory.map((order, idx) => (
                <div key={idx} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <p className="text-[10px] font-black text-[#ea580c] uppercase">{order.timestamp}</p>
                    </div>
                    <span className="text-[9px] px-2 py-0.5 rounded-full font-black bg-green-100 text-green-600">
                      สำเร็จ
                    </span>
                  </div>
                  <div className="space-y-1 border-t pt-2">
                    {order.items.map((item: any, i: number) => (
                      <div key={i} className="flex justify-between text-[11px] font-bold">
                        ...
                        <span className="text-slate-900">{item.qty} {item.unit}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-1">
                ...
                <div className="flex flex-nowrap items-center bg-[#ea580c] rounded-lg p-1 shadow-md gap-0.5">
                  <button
                    ...
                    className="text-white font-black text-lg flex items-center justify-center w-8 h-8 rounded-lg active:scale-95 flex-shrink-0"
                  >
                    −
                  </button>
                  <input 
      ...
      {cartItems.length > 0 && activeTab !== "📜 ประวัติการสั่ง" && (
    ...
    <div className="min-h-screen bg-white flex flex-col font-sans text-slate-900">
      ...
      <div className="flex-1 p-6 max-w-md mx-auto w-full space-y-4">
        {cartItems.map(id => (
          <div key={id} className="flex justify-between items-center border-b-2 border-slate-50 pb-4">
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">สต็อกเดิม: {cart[id].stock || 0} {cart[id].unit}</p>
            </div>
        ...
        <div className="pt-4">
          ...
          <textarea 
            ...
            className="w-full bg-slate-50 p-4 rounded-2xl text-sm font-bold border-2 border-slate-100 focus:border-orange-500 outline-none resize-none"
      ...
      <div className="p-6 bg-slate-50 border-t sticky bottom-0">
        <div className="max-w-md mx-auto">
          ...
          <button onClick={() => setStep(2)} className="w-full mt-4 text-slate-400 font-bold text-[10px] uppercase tracking-widest">กลับไปแก้ไขรายการ</button>
        </div>
      </div>
    </div>
  );
